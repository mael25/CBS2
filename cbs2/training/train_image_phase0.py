...
def train_or_eval(coord_converter, criterion, net, teacher_net, data, optim, is_train, config, is_first_epoch):
    ...
    for i, (rgb_image, birdview, location, command, speed, wp_method) in iterator:
        ...
        teac_location = (_teac_location + 1) * coord_converter._img_size/2
...
def train(config):
    ...
    teacher_net = BirdViewPolicyModelSS(backbone=teacher_config['model_args']['backbone'],
                                        input_channel=teacher_config['model_args']['input_channel'],
                                        seg=teacher_config['data_args']['segmentation']).to(
        config['device'])
