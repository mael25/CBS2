...
class ModuleWorld(object):
    ...
    def get_hero_measurements(self):
        ...
        pos_transform = self.hero_cam.get_transform()
        return {
                'position': np.float32([pos.x, pos.y, pos.z]),
                'orientation': np.float32([ori.x, ori.y, ori.z]),
                'position_cam': np.float32([pos_transform.location.x,
                                            pos_transform.location.y,
                                            pos_transform.location.z]),
                'orientation_cam': np.float32([pos_transform.rotation.pitch,
                                               pos_transform.rotation.yaw,
                                               pos_transform.rotation.roll]),
...
class Wrapper(object):
    ...
    @classmethod
    def init(cls, client, world, carla_map, player, hero_cam=None):
        ...
        world_module = ModuleWorld(MODULE_WORLD, client, world, carla_map,
                                   player, hero_cam=hero_cam)
