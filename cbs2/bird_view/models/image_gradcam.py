...
class ImagePolicyModelSS(common.ResnetBase):
    ...
    def forward(self, image, velocity=5.0, command=3):
        if self.warp:
        ...
        velocity = (torch.ones([b, 128, kh, kw]) * velocity).cuda()
        command = (torch.ones([b, 1]) * command).cuda()
        ...
        print(location_pred)
        var_pred = torch.var(location_pred)
        inv_var = torch.div(torch.ones(var_pred.size()).cuda(), var_pred)
