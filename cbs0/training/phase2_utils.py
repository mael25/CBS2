...
def load_image_model(backbone, ckpt, ppm_bins, fpn, device='cuda'):
...
def load_birdview_model(backbone, ckpt, device='cuda'):
    teacher_net = BirdViewPolicyModelSS(backbone, all_branch=True,
                                        input_channel=13, seg=True).to(device)
...
class ReplayBuffer(torch.utils.data.Dataset):
    ...
    def __getitem__(self, _idx):
        ...
        birdview_img = self.birdview_transform(np.ascontiguousarray(birdview_img))
    ...
    def add_data(self, rgb_img, cmd, speed, target, segmentation, weight):
        self.normalized = False
        segmentation = np.moveaxis(segmentation, 0, -1)
        self._data.append((rgb_img, cmd, speed, target, segmentation))
