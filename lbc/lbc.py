...
class LBC:
    ...
    def train_bev(self, lbls, spds, locs, cmds):
        print(f'Shape input LBC: {lbls.shape}\n')
        pred_locs = self.bev_model(lbls, spds).view(-1,self.num_cmds,self.T,2)

        # Scale pred locs
        pred_locs = (pred_locs+1) * self.crop_size/2
        print(f'Shape pred_locs CBS: {_numpy(pred_locs).shape}\n')
        print(f'Adv. Shape pred_locs CBS: {_numpy(pred_locs.gather(1, cmds[:,None,None,None].repeat(1,1,self.T,2)).squeeze(1)).shape}\n')
        print(f'Shape locs CBS: {_numpy(locs).shape}\n')
