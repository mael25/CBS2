...
def unproject(output, world_y=1.4, fov=90):
...
def get_ts(model_pred):
    DT = 0.1
    world_pred = unproject(model_pred)
    targets = [(0, 0)]
    ...
    target_speed = np.linalg.norm(targets[:-1] - targets[1:], axis=1).mean() / (5 * DT)
