...
class OrnsteinUhlenbeckActionNoise:
    ...
    def __init__(self, mu=0, sigma=0.04, theta=0.04, dt=0.1, x0=None):
        self.theta = theta
        self.mu = mu
        self.sigma = sigma
        self.dt = dt
...
class AutoCollector(AutonomousAgent):
    ...
    def setup_environment(self):
        ...
        self.set_local_planner_global_plan(self._local_planner, self._global_plan_world_coord)
    ...
    def run_step(self, input_data, timestamp):
        ...
        tls = int(f'{int(np.count_nonzero(lbl[...,3]) > 0 )+1}{int(is_relevant_red_traffic_light_red)+1}{int(light_state)+1}')
        #print(tls)
        ...
        #current_waypoint=self._local_planner._current_waypoint
        current_waypoint2 = self._map.get_waypoint(self._vehicle.get_location())
        #print(current_waypoint)
        print(current_waypoint2)
        #is_junction = current_waypoint2.is_junction
        #print(f'Junction: {is_junction}'
        next_waypoint, cmd = self._local_planner._waypoints_queue[0]
        print(next_waypoint)
        print(cmd)
        #next_waypoint =self._local_planner._current_waypoint.next(self._local_planner._sampling_radius)[0].next(self._local_planner._sampling_radius)[0]
        #print(next_waypoint)
        #target_vector = np.array([next_waypoint.transform.location.x - current_waypoint.transform.location.x, next_waypoint.transform.location.y - current_waypoint.transform.location.y])
        #print(np.linalg.norm(target_vector))
        #cmd = _compute_connection(current_waypoint,next_waypoint)
        #print(cmd)
        print('>------------------------------------------------------')
        #print(cmd)
    ...
    def set_local_planner_global_plan(self, local_planner, new_plan):
        """
        ...
        """
        # Reset the queue
        local_planner._waypoints_queue.clear()

        for wp_transform, road_option in new_plan:
            wp = self._map.get_waypoint(wp_transform.location)
            local_planner._waypoints_queue.append([wp, road_option])
        local_planner._target_road_option = new_plan[0][1]
        print(local_planner._target_road_option)

        # and the buffer
        local_planner._waypoint_buffer.clear()
        for _ in range(local_planner._buffer_size):
            if local_planner._waypoints_queue:
                local_planner._waypoint_buffer.append(
                    local_planner._waypoints_queue.popleft())
            else:
                break

        local_planner._global_plan = False
